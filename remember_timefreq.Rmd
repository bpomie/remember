...
```{r include=FALSE}
ggsave("plots/figuregammapres1.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figuregammapres2.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figuregammapres_summary.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figuregammaoccl1.png", plot = last_plot(), width = 17, height = 10, units = c("cm"))
```
...
```
```{r include=FALSE}
ggsave("plots/figuregammaoccl2.png", plot = last_plot(), width = 18, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figuregammaoccl_summary.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
...
```
```{r include=FALSE}
ggsave("plots/figurealphapres2.png", plot = last_plot(), width = 18, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figurealphapres_summary.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
...
Plot.
```{r}
plotalphaocclusion <- longalphaocclusion %>%
  group_by(Category, Hemisphere) %>%
  get_summary_stats(occlusion_alpha,type = "mean_se")

palphaocclusion <- ggplot(data = plotalphaocclusion,
                          aes(x=Category, y = mean, color=Hemisphere, fill=Hemisphere,  group=Hemisphere)) +
   theme_light() +
          theme(strip.text = element_text(size = 12),
                panel.grid.major.y = element_line(colour="grey75"),
                panel.grid.minor.y = element_blank(),
                ...
                legend.position="bottom",
                axis.title.x = element_blank()) +
  geom_bar(stat="identity", position=position_dodge(),width=.7,alpha = 1) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1,position=position_dodge(.7)) +
  scale_fill_manual(values = c("black","gray50")) +
  scale_color_manual(values = c("gray50","gray80")) +
  scale_y_continuous(name=alpha~"-band activity("~mu~"V)", limits=c(-.6, .6)) 
...
boxalphaocclusion <- ggplot(data = longalphaocclusion,
                            aes(x=Category, y=occlusion_alpha, fill = Hemisphere, color = Hemisphere)) +
  theme_light() +
          theme(strip.text = element_text(size = 12),
                panel.grid.major.y = element_line(colour="grey75"),
                panel.grid.minor.y = element_blank(),
                ...
                legend.position="bottom",
                axis.title.x = element_blank()) +
  geom_boxplot(aes(fill=Hemisphere), position = position_dodge(.9), 
               alpha = 1, outlier.shape = NA, width = .5) +
  geom_jitter(aes(color=Hemisphere),position = position_jitterdodge(.25), size = 1) +
  scale_y_continuous(name=alpha~"-band activity("~mu~"V)", limits=c(-2, 2)) +
  scale_fill_manual(values = c("black","gray50")) +
  scale_color_manual(values = c("gray50","gray80"))
...
```{r include=FALSE}
ggsave("plots/figurealphaoccl2.png", plot = last_plot(), width = 18, height = 10, units = c("cm"))
```
...
```{r include=FALSE}
ggsave("plots/figurealphaoccl_summary.png", plot = last_plot(), width = 17.5, height = 10, units = c("cm"))
```
